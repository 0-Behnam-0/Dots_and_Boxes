...
def game_map():
    ...
    for i in range(rows):
        ...
        print()
        for k in range(columns):
            if str(base_elements[counter]).isnumeric():     #if default value
                if k+1 == columns:      #if printing last column
                   print(str(base_elements[counter])+"       ", end="") if base_elements[counter] >= 10 else print("0"+str(base_elements[counter])+"  "+house_id+"   ", end="")
                else:
            ...
            else:       #if vertical value chosed
                if k+1 == columns:      #if printing last column
                    print("¦       ", end="")
                else:
                    if str(bonus_houses[bonus_houses_counter]).isnumeric():
                        if bonus_houses_counter>=9:
                            print("¦  "+str(bonus_houses[bonus_houses_counter])+"   ", end="")
                        else:
                            print("¦  0"+str(bonus_houses[bonus_houses_counter])+"   ", end="")
                    else:
                        print("¦   "+str(bonus_houses[bonus_houses_counter])+"   ", end="")
                    bonus_houses_counter += 1
                    ...
                    #                                                     if str(bonus_houses[bonus_houses_counter]).isnumeric() else " "+str(bonus_houses[bonus_houses_counter])
